export function useCart() {
  ...
  const total = computed(() => items.value.reduce((sum, i) => sum + parseFloat(i.product.price) * i.quantity, 0))

  const count = computed(() => items.value.reduce((sum, i) => sum + i.quantity, 0))

  const isEmpty = computed(() => items.value.length === 0)

  const toOrderItems = computed(() => items.value.map((i) => ({ product_id: i.product.id, quantity: i.quantity })))
